...
export default function ListProductComponent({ arrProduct }) {
	...
	const renderTitleSortByPrice = () => {
		return arrTitleSortBy.map(sortBy => (
			<div
				key={sortBy.id}
				onClick={() => handleSortByPrice(sortBy.id)}
				className={`cursor-pointer
                   ${sortBy.isLast ? '' : 'border-b'} py-1.5
                   ${sortByActive === sortBy.id
						? 'text-neutral-500 text-rose-600'
						: ''
					}
                   `}>
				{sortBy.content}
			...
			<div className='flex flex-wrap'>
				{sizes.map(size => (
					<div
						...
						className={`${sizeActive === size.id ? 'border-black' : ''
							}
	...
	const renderProductList = () => {
		return (
			<div
				className={`flex flex-wrap ${result?.length > 0 ? '' : 'justify-center'
					}`}>
				{result?.length > 0 ? (
					result?.map(pro => (
						<div key={pro._id} className='w-1/4'>
	...
	return (
		<Container>
			<div className='flex justify-between items-end border-b-2 mb-4'>
				<Box className='flex'>{renderTitleFilter()}</Box>
				<div
				...
				<Drawer
					...
					<Box className='w-96'>
						...
						<ItemDrawer
							title='Sort By'
							renderFuntion={renderTitleSortByPrice}
						/>
						<ItemDrawer
							title='Kích cỡ'
							renderFuntion={renderSizeList}
						/>
						<ItemDrawer
							title='Loại Sản Phẩm'
							renderFuntion={renderCatProductList}
						/>
						<ItemDrawer
							title='MÀU'
							renderFuntion={renderColorList}
						/>
					</Box>
				</Drawer>
			</div>
